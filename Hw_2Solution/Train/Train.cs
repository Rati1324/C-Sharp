...
namespace Train
{
    public class Train
    {
        ...
        {
            if (NumOfCoach <= 0 || PersonPerCoach <= 0) throw new Exception("No negative number allowed");
            return NumOfCoach * PersonPerCoach;
        }

        public static float Passengers(float EnergyPerKm, float Distance)
        {
            if (EnergyPerKm <= 0 || Distance <= 0) throw new Exception("No negative number allowed");
